$(document).ready(function () {
	...
	var buttonsFilter = function(elem) { return elem.classList && (elem.classList.contains("header-link") || elem.classList.contains("cont")); };
	...
	// Handle window resize to reset menu items visibility and format
	$(window).resize(function() {
		if ($(window).width() > 940) {
			$('.header__menu ul').removeClass('open').show();
			$('.header').removeClass('menu-open');
			$('.menu-button').removeClass('menu-open');
		} else {
			$('.header__menu ul').hide(); // Ensure dropdown menu is hidden initially in mobile view
		}
	}).trigger('resize'); // Trigger resize to ensure correct state on load

	// Close menu on item click
	$('.header__menu ul a').on('click', function () {
		if ($(window).width() < 940) {
			$('.header__menu ul').removeClass('open').hide();
			$('.header').removeClass('menu-open');
			$('.menu-button').removeClass('menu-open');
		}
	});
